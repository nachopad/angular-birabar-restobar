...
pedidoCtrl.getPedidos = async (req, res) => {
    const pedidos = await Pedido.find().populate('cliente').populate('detalleProductos').populate('calificacion');
    res.json(pedidos);
}

pedidoCtrl.getPedidoId = async (req, res) => {
    const pedido = await Pedido.findById({ _id: req.params.id }).populate('cliente').populate('detalleProductos').populate('calificacion');
    res.json(pedido);
}
...
pedidoCtrl.getPedidosEstado = async (req, res) => {
    try {
        ...
        const pedidos = await Pedido.find({ estado }).populate('cliente').populate('detalleProductos').populate('calificacion');
        res.json(pedidos);
    } catch (error) {
        res.status(400).json({
            status: '0',
